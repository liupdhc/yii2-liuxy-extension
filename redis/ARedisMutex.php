 ...
 * <pre>
 * $mutex = new ARedisMutex("someOperation");
 * $mutex->afterLock = function(Event $event) {
 *  echo "Locked!\n";
 *    // do some processing here
 *  $event->sender->unlock(); // finally, unlock the mutex
 * };
 * $mutex->afterUnlock = function(Event $event) {
 *  echo "Unlocked!";
 * }
...
class ARedisMutex extends ARedisEntity {
    ...
    public function onAfterLock ($event) {
        $this->_connection->trigger ("onAfterLock" , $event);
    }
    ...
    public function onBeforeUnlock ($event) {
        $this->_connection->trigger ("onBeforeUnlock" , $event);
    }
    ...
    public function onAfterUnlock ($event) {
        $this->_connection->trigger("onAfterUnlock" , $event);
    }
